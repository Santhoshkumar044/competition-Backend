...
export async function updateEvent(req, res) {
  ...
  try {
    ...
    if (updates.startTime) updatePayload.startTime = new Date(updates.startTime);
    if (updates.endTime) updatePayload.endTime = new Date(updates.endTime);
    if (updates.EventDate) updatePayload.EventDate = updates.EventDate;
